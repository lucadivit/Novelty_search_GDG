...
import os, math, shutil, random, time, copy, neat
...
import environment.maze_environment as maze
import agent.agent as agent
import agent.novelty_archive as archive
...
mazes_path = "mazes/"
out_dir = "out/maze_ns/"
args["maze"] = mazes_path + "medium"
args["generations"] = 100
args["trials"] = 1
args["ns_threshold"] = 6.0
args["location_sample_rate"] = 4000
args["width"] = 300
args["height"] = 150


if not (args["maze"] == 'mazes/medium' or args["maze"] == 'mazes/hard' or args["maze"] == 'mazes/simple'):
    exit(1)

start = time.time()
# set random seed
seed = 757175 # str(time.time()).replace(".", "")[-6:]
seed = int(seed)
random.seed(seed)
...
def run_experiment(config_file, maze_env, novelty_archive, trial_out_dir, args=None, n_generations=100,
    ...
    if save_results or show_results:
        node_names = {-1: 'RF_R', -2: 'RF_FR', -3: 'RF_F', -4: 'RF_FL', -5: 'RF_L', -6: 'RF_B',
                      ...
                      0: 'ANG_VEL', 1: 'VEL'}
        visualize.draw_net(config, best_genome, view=show_results, node_names=node_names, directory=trial_out_dir,
                           fmt='svg')
        if args is None:
